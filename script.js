...
// Setup Cursor Spotlight Hover Effect for IPv4 & IPv6 Cards
function initSpotlightCards() {
  ...
  cards.forEach(card => {
    card.addEventListener('mousemove', (e) => {
      ...
      // Update cursor spotlight glare coordinates (flat card, zero 3D tilt)
...
async function handleDownloadReport() {
  ...
  try {
    ...
    // Format dynamic file name with user's detected IP address
    const userIp = state.ipv4 || state.ipv6 || 'Client';
    const safeIpForFilename = String(userIp).replace(/[:]/g, '_').trim();
    const pdfFilename = `IP_Finder_Report_${safeIpForFilename}.pdf`;
